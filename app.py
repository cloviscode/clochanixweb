...
def admin_add_project():
    if request.method == 'POST':
        try:
            ...
            # Create project data structure as before
            project_data = {
            ...
            # Process multiple image uploads (up to 4)
            image_count = 0
            max_images = 4
            ...
            # Process video upload (single video)
            ...
            # Add to project list and redirect
            projects_data[project_key].insert(0, project_data)
            flash(f'Project added successfully with {len(project_data["media"]["images"])} images and {len(project_data["media"]["videos"])} videos!', 'success')
            return redirect(url_for('admin_projects'))
